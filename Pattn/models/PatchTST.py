...
# Implements patch extraction, embedding, multi-head attention, and prediction head.
...
class AttentionLayer(nn.Module):
    """
    Multi-head attention layer with input projections and output projection.
    Wraps an inner attention mechanism (e.g., FullAttention).
    """
    ...
    def forward(self, queries, keys, values, attn_mask, attn_bias):
        ...
        # Inner attention returns output and attention weights
...
class FullAttention(nn.Module):
    """
    Standard full self-attention mechanism (O(L^2) complexity).
    Supports optional masking and attention bias.
    """
...
class EncoderLayer(nn.Module):
    """
    Transformer encoder layer: attention + feedforward + normalization.
    """
...
class Encoder(nn.Module):
    """
    Stacked encoder layers for patch-based transformer.
    Optionally includes convolutional layers and normalization.
    """
...
class PatchTST(nn.Module):
    ...
    def __init__(self, configs, device):
        ...
        # Patch embedding (no time embedding)
        ...
        self.encoder = Encoder(
            ...
            # Use BatchNorm for normalization
            norm_layer=torch.nn.BatchNorm1d(configs.d_model)
        )
        
        # Final projection: flatten all patch features and predict output sequence
    ...
    def forward(self, x_enc, itr):
        ...
        Input: x_enc [Batch, SeqLen, Channels]
        Output: [Batch, PredLen, Channels]
        """
        B, L, M = x_enc.shape
        ...
        # Rearrange for patch extraction: [Batch, Channels, SeqLen]
        x_enc = rearrange(x_enc, 'b l m -> b m l')
        # Extract patches: [Batch, Channels, NumPatches, PatchSize]
        x_enc = x_enc.unfold(dimension=-1, size=self.patch_size, step=self.stride)
        # Merge batch and channel for transformer: [(Batch * Channels), NumPatches, PatchSize]
        ...
        # For compatibility, also update x_enc (not used)
        x_enc = enc_out * stdev
        x_enc = enc_out + means

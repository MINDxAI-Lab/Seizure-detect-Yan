...
# Use file-based repo root to avoid CWD-related issues
REPO_ROOT = Path(__file__).resolve().parents[1]
sys.path.append(str(REPO_ROOT))
# Use the matched file markers for TUSZ v2.0.3
...
class SeizureDataset(Dataset):
    def __init__(
        ...
        # Filter file_tuples to only include clips with correct sequence length
        if self.preproc_dir is not None:
            print(f"Filtering clips to match expected sequence length of {max_seq_len}...")
            filtered_tuples = []
            skipped_count = 0
            for h5_fn, seizure_label in self.file_tuples:
                try:
                    with h5py.File(os.path.join(self.preproc_dir, h5_fn), 'r') as hf:
                        clip_shape = hf['clip'].shape
                        if clip_shape[0] == max_seq_len:
                            filtered_tuples.append((h5_fn, seizure_label))
                        else:
                            skipped_count += 1
                except Exception as e:
                    print(f"Error reading {h5_fn}: {e}")
                    skipped_count += 1
            
            print(f"Filtered {len(self.file_tuples)} files to {len(filtered_tuples)} valid clips. Skipped {skipped_count} clips.")
            self.file_tuples = filtered_tuples
    ...
    def __getitem__(self, idx):
        ...
        # Extract clip index from filename like: aaaaafcf_s009_t006_6.h5
        clip_idx = int(h5_fn.split('_')[-1].split('.h5')[0])

        # Extract EDF base name from preprocessed filename
        # Format: aaaaafcf_s009_t006_6.h5 -> aaaaafcf_s009_t006.edf
        parts = h5_fn.split('_')
        edf_base_name = '_'.join(parts[:-1]) + '.edf'  # Remove clip index and add .edf
        
        edf_file = [file for file in self.edf_files if os.path.basename(file) == edf_base_name]
        
        if len(edf_file) == 0:
            # Fallback: try to find any file containing the patient and session info
            patient_session = '_'.join(parts[:2])  # e.g., aaaaafcf_s009
            edf_file = [file for file in self.edf_files if patient_session in os.path.basename(file)]
        
        assert len(edf_file) >= 1, f"Found {len(edf_file)} matches for {edf_base_name}. Available files: {[os.path.basename(f) for f in self.edf_files[:5]]}"
        edf_file = edf_file[0]  # Take the first match

        # preprocess
        if self.preproc_dir is None:
            # For raw processing: need to reconstruct the resampled file path
            # Remove clip index and .edf extension if present
            base_parts = '_'.join(parts[:-1])
            if base_parts.endswith('.edf'):
                base_parts = base_parts[:-4]  # Remove .edf extension
            resample_base_name = base_parts + '.h5'
            resample_sig_dir = os.path.join(self.input_dir, resample_base_name)

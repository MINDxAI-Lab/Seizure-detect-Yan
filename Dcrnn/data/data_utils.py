...
def getSeizureTimes(file_name):
    ...
    """
    csv_file = file_name.split(".edf")[0] + ".csv_bi"
    ...
    with open(csv_file) as f:
        for line in f.readlines():
            ...
            # Skip header line
            if "channel" in line and "start_time" in line:
                continue
                
            parts = line.strip().split(',')
            if len(parts) >= 4:  # Ensure we have enough columns
                label = parts[3].strip()
                if "seiz" in label.lower():  # if seizure
                    # seizure start and end time
                    seizure_times.append(
                        [
                            float(parts[1].strip()),  # start_time
                            float(parts[2].strip()),  # stop_time
                        ]
                    )
...
def getSeizureClass(file_name, target_labels_dict=None, file_type="edf"):
    ...
    annotation_file = ""
    if file_type == "edf":
        annotation_file = file_name[:-4] + ".csv_bi"
    elif file_type == "csv":
    ...
    with open(annotation_file) as f:
        for line in f.readlines():
            ...
            if line.startswith('#') or line.strip() == "" or "channel" in line:
                continue
            ...
            if len(parts) >= 4:  # Ensure we have enough columns
                label = parts[3].strip()
                if any(
                    s in label.lower() for s in target_labels
                ):  # if this is one of the seizure types of interest
                    seizure_str = [s for s in target_labels if s in label.lower()]
                    if seizure_str:
